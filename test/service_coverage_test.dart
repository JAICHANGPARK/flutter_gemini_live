...
void main() {
  group('ApiClient', () {
    test('post sends JSON to the default endpoint and decodes success bodies', () async {
      ...
      expect(
        client.requests.single.headers,
  ...
  group('LiveService', () {
    ...
    test('handleWebSocketData parses string and binary payloads', () {
      ...
      service.handleWebSocketDataForTesting(
        utf8.encode(
          '{"serverContent":{"modelTurn":{"parts":[{"inlineData":{"mimeType":"audio/pcm","data":"abc"}}]}}}',
        ),
        callbacks,
      );
    ...
    test('buildSetupMessage normalizes defaults and rejects unsupported parameters', () {
    ...
    test('connect builds URI and headers, sends setup, and returns a session', () async {
          ...
          tools: [
            Tool(
              functionDeclarations: [
                FunctionDeclaration(name: 'lookup'),
              ],
            ),
          ],
      ...
      expect(seenHeaders['x-goog-api-client'], 'google-genai-sdk/1.42.0 dart/9.9');
      expect(seenHeaders['user-agent'], 'google-genai-sdk/1.42.0 dart/9.9');

      final sentSetup = jsonDecode(channel.sentMessages.single as String)
          as Map<String, dynamic>;
      expect(sentSetup['setup']['model'], 'models/gemini-live-test');
      expect(sentSetup['setup']['system_instruction']['parts'][0]['text'], 'system');
      expect(
    ...
    test('connect still reaches the ephemeral warning branch on non-alpha versions', () async {
      final service = LiveService(
  ...
  group('LiveSession', () {
    test('send methods encode the expected message payloads', () async {
      ...
      session.sendClientContent(
        turns: [Content(parts: [Part(text: 'turn')])],
        turnComplete: false,
      );
      ...
      expect(decoded[0]['clientContent']['turns'][0]['parts'][0]['text'], 'hello');
      ...
      expect(decoded[4]['realtimeInput']['activity_start'], isA<Map<String, dynamic>>());
      expect(decoded[4]['realtimeInput']['activity_end'], isA<Map<String, dynamic>>());
      ...
      expect(decoded[8]['realtimeInput']['activity_start'], isA<Map<String, dynamic>>());
      expect(decoded[9]['realtimeInput']['activity_end'], isA<Map<String, dynamic>>());
      expect(decoded[10]['toolResponse']['function_responses'][0]['response'], {'ok': true});
      expect(decoded[11]['toolResponse']['function_responses'][0]['response'], {'value': 3});
